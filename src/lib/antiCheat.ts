...
export class AntiCheatMonitor {
  ...
  private lastKeystroke = Date.now();
  ...
  private listenVisibilityChange() {
    const handler = () => {
      if (document.hidden) {
        this.log('TAB_SWITCH', 'medium', 'Candidate switched tabs');
      }
    };
  ...
  private listenKeyboard() {
    const handler = (e: KeyboardEvent) => {
      ...
      if (ctrl && e.key === 'c') {
        this.log('COPY_DETECTED', 'medium', 'Ctrl+C in editor');
      }
      if (ctrl && e.key === 'v') {
        this.log('PASTE_DETECTED', 'high', 'Ctrl+V in editor');
      }
      if (e.key === 'PrintScreen') {
        this.log('SCREENSHOT_ATTEMPT', 'low');
      }
  ...
  private detectRapidTyping() {
    ...
    if (this.typingBuffer.length > 16) {
      ...
      if (wpm > 200) {
        this.log('RAPID_TYPING', 'high', `Estimated ${Math.round(wpm)} WPM — possible paste`);
        this.typingBuffer = [];
      }
    }
  }
  ...
  private listenFullscreenChange() {
    const handler = () => {
      if (!document.fullscreenElement) {
        this.log('FULLSCREEN_EXIT', 'medium', 'Candidate exited fullscreen');
      }
    };
  ...
  private startDevToolsPolling() {
    const check = () => {
      if (
        ...
        this.log('DEVTOOLS_OPENED', 'critical', 'DevTools size heuristic triggered');
      }
    };
  ...
  private resetIdle() {
    ...
    this.idleTimer = setTimeout(() => {
      this.log('IDLE_TOO_LONG', 'low', 'No keystrokes for over 5 minutes');
    }, 5 * 60 * 1000);
  }
  ...
  private log(type: CheatEventType, severity: CheatSeverity, detail?: string) {
    ...
    if (severity === 'medium' || severity === 'high') {
      this.config.onWarning('Suspicious activity detected. This has been logged and will be reviewed by the recruiter.');
    }
  }

  private flagSession() {
    this.flagged = true;
    this.log('SESSION_FLAGGED', 'critical', 'Session flagged due to repeated violations');
